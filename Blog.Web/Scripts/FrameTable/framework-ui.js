...
$.currentWindow = function () {
    var iframeId = top.$(".ChinaEdu_iframe:visible").attr("id");
...
$.modalHtmlDialog = function (options) {
    var defaults = {
        id: null,
        title: '系统窗口',
        width: "100px",
...
$.modalClose = function (iframeId) {
    ...
    var index = top.layer.getFrameIndex(iframeId) || top.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
...
$.modalHtmlClose = function () {

    var $IsdialogClose = top.$("[type=page]").find(".layui-layer-btn").find('a') ;
 
    for (var i = 0; i < $IsdialogClose.length; i++) {

        if ($($IsdialogClose[i]).html() == "关闭") {
            //alert($($IsdialogClose[i]).html())
            $IsdialogClose[i].click();
        }
...
$.fn.formSerialize = function (formdate) {
    ...
    element.find('input,select,textarea').each(function (r) {
        ...
        switch (type) {
            case "checkbox":
                postdata[id] = $this.is(":checked") == true ? 1 : 0;
                break;
            default:
...
$.fn.authorizeButton = function () {
    ...
    var moduleId = top.$(".chinaEdu_iframe:visible").attr("id");
